...
if not os.path.exists(os.path.join(results_dir, 'patch_csvs')):
    os.mkdir(os.path.join(results_dir, 'patch_csvs'))
...
if 'z_score' in norm_type:
    ...
    for im_fname in norm_fnames:
        ...
        initial_im = tifffile.imread(os.path.join(normalization_dir, im_fname))
...
for im_fname in tqdm(image_fnames):
    ...
    final_patch_frame.to_csv(os.path.join(results_dir, 'patch_csvs', 'patch_info.csv'), index=False)
